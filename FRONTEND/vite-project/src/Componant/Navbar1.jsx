...
import {
    AppBar, Toolbar, Box, Typography, Button, Stack,
    Container, IconButton, Menu, MenuItem, Fade, Divider, Avatar
} from '@mui/material';
import {
    ChevronDown, Layout, Settings, LogOut, User, 
    Plus, Bell, ShieldCheck, Zap, BarChart3, 
    Box as BoxIcon, Globe, Briefcase, LogIn,
    Rocket, ShoppingBag, HeartPulse, Globe2, Shield
} from 'lucide-react';
...
const Navbar1 = () => {
    ...
    return (
        <AppBar position="sticky" elevation={0} className="dual-tone-navbar">
                ...
                <Toolbar disableGutters sx={{ height: 72, justifyContent: 'space-between' }}>
                    ...
                    <Stack direction="row" alignItems="center" spacing={4}>
                        <Link to="/" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 12 }}>
                            <Box sx={{ 
                                bgcolor: '#6366f1', p: 0.8, borderRadius: '10px', display: 'flex',
                        ...
                        <Stack direction="row" spacing={1} sx={{ display: { xs: 'none', lg: 'flex' } }}>
                            ...
                            {/* <Link to="/">
                            <Button className="dual-tone-nav-btn">Live chats</Button>
                            </Link> */}
                            <Link to="/ApiDocs">
                            <Button className="dual-tone-nav-btn">API Docs</Button>
                            </Link>
                            <Link to="/Docs">
                            <Button className="dual-tone-nav-btn">Docs</Button>
                            </Link>
                            {/* <Button 
                                className="dual-tone-nav-btn"
                            ...
                            {/* <Link to="/ReportPage">
                            <Button className="dual-tone-nav-btn">Reports</Button>
                            </Link> */}
                        </Stack>
                    </Stack>
                    ...
                    <Stack direction="row" alignItems="center" spacing={2}>
                        <IconButton sx={{ color: '#94a3b8' }}><Bell size={20} /></IconButton>
            {!isLoggedIn ? (
              <Link to="/Login">
                <Button variant="contained" className="dual-tone-action-btn">
                  Login
                       ...
                       <Link to = "/SignUp"><Button
                            variant="contained"
                            className="dual-tone-action-btn"
                        ...
                        {/* <IconButton onClick={handleOpen('profile')} sx={{ p: 0.5 }}>
                            <Avatar sx={{ 
                                width: 38, height: 38, bgcolor: '#334155', 
                                border: '2px solid #475569', color: '#818cf8', fontSize: 14, fontWeight: 700 
                            }}>JD</Avatar>
                        </IconButton> */}
                    </Stack>
                </Toolbar>
            ...
            >
                <Typography variant="caption" sx={{ color: '#94a3b8', fontWeight: 800, px: 1.5, mb: 1, display: 'block', letterSpacing: 1 }}>MODULES</Typography>
                <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1 }}>
                    <MegaItem to="/AdminDashboard" icon={<Layout color="#6366f1" />} title="Admin Panel" desc="System Control" />
                    <MegaItem to="/HodDashboard" icon={<ShieldCheck color="#10b981" />} title="HOD Desk" desc="Approvals" />
                    <MegaItem to="/TechnicianDashboard" icon={<BarChart3 color="#f59e0b" />} title="Ops Center" desc="SLA Tracking" />
                    <MegaItem to="/RequestorDashboard" icon={<User color="#8b5cf6" />} title="Employee" desc="My Requests" />
                </Box>
            </Menu> */}
            ...
            >
                <Box sx={{ px: 2, py: 1.5 }}>
                    <Typography variant="subtitle2" fontWeight={800} color="#0f172a">John Doe</Typography>
                    <Typography variant="caption" color="#64748b">Super Admin</Typography>
                </Box>
                <Divider sx={{ my: 1 }} />
...
const ProfileItem = ({ icon, label, color = '#475569' }) => (
    <MenuItem sx={{ borderRadius: '8px', gap: 1.5, py: 1, color: color }}>
        {icon} <Typography variant="body2" fontWeight={600}>{label}</Typography>
    </MenuItem>
);
